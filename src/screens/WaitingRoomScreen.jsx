...
export default function WaitingRoomScreen() {
  ...
  const claimedCount = playerList.filter(p => p?.claimed === true).length
  const allReady = claimedCount === 4
  const s = room.settings || {}
  const isCouples = s.playerMode === 'couples'

  const handleStart = async () => { await updateRoomStatus(code, 'playing') }
  ...
  return (
    <div style={{ minHeight: '100vh', padding: 24, maxWidth: 480, margin: '0 auto' }}>
          ...
          {playerList.map((p, i) => {
            ...
            return (
              <div key={i} style={{
                ...
                borderRadius: 12, border: `1px solid ${claimed ? 'rgba(37,99,235,0.2)' : 'var(--border)'}`,
              }}>
                <div style={{
                  width: 38, height: 38, borderRadius: '50%',
                  background: claimed ? 'var(--blue)' : 'var(--surface)',
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  fontFamily: 'Outfit, sans-serif', fontWeight: 700, fontSize: 15,
                  color: claimed ? '#fff' : 'var(--text-secondary)',
                  flexShrink: 0, border: `2px solid ${claimed ? 'transparent' : 'var(--border)'}`,
                }}>
                  {p ? (p.name?.[0] || '?').toUpperCase() : i + 1}
                </div>
                <div style={{ flex: 1 }}>
                  <p style={{ fontWeight: 600, fontSize: 15, fontFamily: 'Outfit, sans-serif' }}>
                    {p?.name ?? <span style={{ color: 'var(--text-secondary)' }}>{t('empty_slot')}</span>}
                  </p>
                  {p && (
                    <p style={{ fontSize: 12, color: 'var(--text-secondary)', marginTop: 1 }}>
                      {claimed ? t('player_joined') : t('player_waiting')}{p.isCreator ? ' · Host' : ''}
                    </p>
                  )}
                </div>
                {isCouples && (
                  <span style={{
                    ...
                    padding: '3px 10px', borderRadius: 6, fontFamily: 'Outfit, sans-serif',
                  }}>
      ...
      {isCreator ? (
        ...
        >
          {allReady ? t('waiting_start') : t('waiting_need_players', { n: 4 - claimedCount })}
        </button>
      ) : (
